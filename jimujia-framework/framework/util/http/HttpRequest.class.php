...
class HttpRequest
{
    ...
    public static function socketPost($url, $post_string, $connectTimeout=10, $readTimeout=10) {
        ...
        $fsock = fsockopen($urlInfo['host'], $urlInfo["port"], $errno, $errstr, $connectTimeout);
        if (false == $fsock) {
        ...
        $in .= "Accept: */*\r\n";
        $in .= "User-Agent: onelee.com API PHP5 Client 1.0 (non-curl)\r\n";
        $in .= "Host: " . $urlInfo["host"] . "\r\n";
    ...
    public static function execCurlHttpRequest($url, $param = array(), $httpMethod = 'POST', $curlOpts = null) {
        ...
        if ($result === false) {
            curl_close($ch);
            //throw new Exception(sprintf('curl_errno=%s, curl_error=%s', curl_errno($ch), curl_error($ch)));
        } elseif (empty($result)) {
     ...
     * @param $url
     * @param array $params
     * @param timeOut HTTP 请求类超时时间
     * @param array $curlOpts curl 参数配置, 允许为空
     * @return mix
     */
    public static function httpPostByCurl($url, $params, $timeOut=30) {
        $ch = curl_init();
        $post = http_build_query($params);
        $url = iconv("GBK", "UTF-8", $url);
        $curl_opts = array(
            CURLOPT_URL => $url, CURLOPT_HEADER => false,
            CURLOPT_RETURNTRANSFER => true, CURLOPT_USERAGENT => 'Mozilla/5.0 (Win94; I);',
            CURLOPT_POST => 1, CURLOPT_POSTFIELDS => $post, CURLOPT_TIMEOUT => $timeOut
        );
