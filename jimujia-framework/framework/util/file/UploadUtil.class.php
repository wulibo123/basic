...
class UploadUtil {
    ...
    private $config = array(
        ...
        'rootPath'      =>  SystemConfig::IMG_UPLOAD_DIR, //保存根路径
        'savePath'      =>  '', //保存路径
        'saveName'      =>  array('uniqid', array('', true)),//array('ukey_next_id', ''), //上传文件命名规则，[0]-函数名，[1]-参数，多个参数使用数组
    ...
    public function upload($files='') {
        ...
        /* 检测上传根目录 */
        if(!$this->_checkRootPath($this->rootPath)){
            return false;
        }

        /* 检查上传目录 */
        if(!$this->_checkSavePath($this->savePath)){
            return false;
        }
        ...
        foreach ($files as $key => $file) {
            ...
            /* 保存文件 并记录保存成功的文件 */
            if ($this->_save($file,$this->replace)) {
    ...
    private function getSubPath($filename) {
        ...
        if ($this->autoSub && !empty($rule)) {
            ...
            if(!empty($subpath) && !$this->_mkdir($this->savePath . '/' . $subpath)){
                return false;
            }
        }
        return $subpath;
