...
class Dispatch {
    ...
    public static function run($moduleName, $actionName, $moduleDir = '', $contentType = '', $jsonpCallback = '', $args = null) {
        ...
        if (!file_exists($filePath)) {
            trigger_error("The file {$filePath} does not exists", E_USER_ERROR);
        }

        include_once $filePath;
        if (!class_exists($className)) {
            trigger_error("The class {$className} does not exists", E_USER_ERROR);
        }
        ...
        if (method_exists($instance, $actionName)) {
            ...
            trigger_error("The Action {$actionName} in Class {$className} does not exists", E_USER_ERROR);
        }
    }
    ...
    protected static function getModuleDirPath($moduleDir) {
        ...
        if (!is_dir($moduleDirPath)) {
            trigger_error("dir {$moduleDirPath} not exists", E_USER_ERROR);
        }
